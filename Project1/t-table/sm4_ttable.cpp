#include "sm4_ttable.h"
#include <cstring>

#define ROL(x, n) (((x) << (n)) | ((x) >> (32 - (n))))

static const u8 Sbox[256] = {
    
};

static u32 TBOX0[256], TBOX1[256], TBOX2[256], TBOX3[256];
static bool table_initialized = false;

static void init_tables() {
    if (table_initialized) return;
    for (int i = 0; i < 256; ++i) {
        u32 b = Sbox[i];
        u32 B = b << 24;
        TBOX0[i] = B ^ ROL(B, 2) ^ ROL(B, 10) ^ ROL(B, 18) ^ ROL(B, 24);

        B = b << 16;
        TBOX1[i] = B ^ ROL(B, 2) ^ ROL(B, 10) ^ ROL(B, 18) ^ ROL(B, 24);

        B = b << 8;
        TBOX2[i] = B ^ ROL(B, 2) ^ ROL(B, 10) ^ ROL(B, 18) ^ ROL(B, 24);

        B = b;
        TBOX3[i] = B ^ ROL(B, 2) ^ ROL(B, 10) ^ ROL(B, 18) ^ ROL(B, 24);
    }
    table_initialized = true;
}

void sm4_key_schedule(const u8 key[16], u32 rk[32]) {
    static const u32 FK[4] = {0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};
    static const u32 CK[32] = {
        0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
        0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
        0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
        0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
    };

    u32 K[36];
    for (int i = 0; i < 4; ++i)
        K[i] = ((u32)key[4*i] << 24) | ((u32)key[4*i+1] << 16) | ((u32)key[4*i+2] << 8) | key[4*i+3];
    for (int i = 0; i < 4; ++i)
        K[i] ^= FK[i];

    for (int i = 0; i < 32; ++i) {
        u32 tmp = K[i+1] ^ K[i+2] ^ K[i+3] ^ CK[i];
        u32 B = 0;
        B |= Sbox[(tmp >> 24) & 0xff] << 24;
        B |= Sbox[(tmp >> 16) & 0xff] << 16;
        B |= Sbox[(tmp >> 8) & 0xff] << 8;
        B |= Sbox[tmp & 0xff];
        u32 T = B ^ ROL(B, 13) ^ ROL(B, 23);
        K[i+4] = K[i] ^ T;
        rk[i] = K[i+4];
    }
}

void sm4_encrypt_block_ttable(const u8 in[16], u8 out[16], const u32 rk[32]) {
    init_tables();
    u32 X[36];
    for (int i = 0; i < 4; ++i)
        X[i] = ((u32)in[4*i] << 24) | ((u32)in[4*i+1] << 16) | ((u32)in[4*i+2] << 8) | in[4*i+3];

    for (int i = 0; i < 32; ++i) {
        u32 tmp = X[i+1] ^ X[i+2] ^ X[i+3] ^ rk[i];
        u32 b0 = (tmp >> 24) & 0xff;
        u32 b1 = (tmp >> 16) & 0xff;
        u32 b2 = (tmp >> 8) & 0xff;
        u32 b3 = tmp & 0xff;
        u32 T = TBOX0[b0] ^ TBOX1[b1] ^ TBOX2[b2] ^ TBOX3[b3];
        X[i+4] = X[i] ^ T;
    }

    for (int i = 0; i < 4; ++i) {
        u32 val = X[35 - i];
        out[4*i] = val >> 24;
        out[4*i+1] = (val >> 16) & 0xff;
        out[4*i+2] = (val >> 8) & 0xff;
        out[4*i+3] = val & 0xff;
    }
}
